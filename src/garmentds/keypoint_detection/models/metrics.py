...
def keypoint_classification(
    ...
    scale_y, scale_x = scale_factor.detach().cpu().numpy()

    detected_keypoints = copy.deepcopy(detected_keypoints)
    ground_truth_keypoints = copy.deepcopy(ground_truth_keypoints)

    sorted_detected_keypoints = sorted(detected_keypoints, key=lambda x: x.probability, reverse=True)[:len(ground_truth_keypoints)]
    pred_coord = np.array([(keypoint.u*scale_x, keypoint.v*scale_y) for keypoint in sorted_detected_keypoints])
    gt_coord = np.array([(keypoint.u*scale_x, keypoint.v*scale_y) for keypoint in ground_truth_keypoints])

    # match detected keypoints to gt keypoints
    pred_coord = pred_coord.reshape(1, -1, 2)
    gt_coord = gt_coord.reshape(-1, 1, 2)
    dist = np.linalg.norm(pred_coord - gt_coord, axis=-1)
    
    for i in range(len(dist)):
        matched = False
        gt_idx, pred_idx = np.unravel_index(np.argmin(dist, axis=None), dist.shape)
        if dist[(gt_idx, pred_idx)] <= threshold_distance + 1e-5:
            matched = True
        classified_keypoint = ClassifiedKeypoint(
            sorted_detected_keypoints[pred_idx].u,
            sorted_detected_keypoints[pred_idx].v,
            sorted_detected_keypoints[pred_idx].probability,
            threshold_distance,
            int(dist[(gt_idx, pred_idx)]),
            matched,
        )
        classified_keypoints.append(classified_keypoint)
        
        dist = np.delete(dist, gt_idx, axis=0)
        dist = np.delete(dist, pred_idx, axis=1)

    return classified_keypoints


def calculate_precision_recall(
    classified_keypoints: List[ClassifiedKeypoint], total_ground_truth_keypoints: List[int]
) -> Tuple[List[float], List[float]]:
    """Calculates precision recall points on the curve for the given keypoints by varying the threshold probability to all detected keypoints
    ...
    gt_keypoint_num = 0
    for i in range(len(total_ground_truth_keypoints)):
        gt_keypoint_num += total_ground_truth_keypoints[i]
    ...
    for keypoint in sorted(classified_keypoints, key=lambda x: x.probability, reverse=True):
        ...
        recall.append(_zero_aware_division(true_positives, gt_keypoint_num))
...
def calculate_mAP(classified_keypoints: List[ClassifiedKeypoint], total_ground_truth_keypoints: List[int]) -> float:
    """
    Calculate mean Average Precision: correctly classfied keypoints / total gt keypoints.
    Classified correctly means the distance between the detected keypoint and the gt keypoint is within the threshold distance.
    """
    correct_num = 0
    gt_keypoint_num = 0
...
def calculate_AKD(
    classified_keypoints: List[ClassifiedKeypoint], 
    total_ground_truth_keypoints: List[int],
    provide_percentile: bool = True,
) -> float:
    """
    Calculate Average Keypoint Distance: mean pixel distance between detected keypoints and their corresponding gt keypoints.
    """
    total_distance = []
    gt_keypoint_num = 0
...
class KeypointAPMetric(Metric):
    ...
    def __init__(self, keypoint_threshold_distance: float, dist_sync_on_step=False):
        ...
        self.add_state("classified_keypoints", default=[], dist_reduce_fx=None)  # list of ClassifiedKeypoints
        self.add_state("total_ground_truth_keypoints", default=[], dist_reduce_fx=None)

    def update(self, detected_keypoints: List[DetectedKeypoint], gt_keypoints: List[Keypoint], scale_factor: torch.Tensor):

        classified_img_keypoints = keypoint_classification(
            detected_keypoints, gt_keypoints, self.keypoint_threshold_distance, scale_factor
        )

        self.classified_keypoints += classified_img_keypoints
        self.total_ground_truth_keypoints.append(len(gt_keypoints))

    def compute(self):
        #p, r = calculate_precision_recall(self.classified_keypoints, self.total_ground_truth_keypoints)
        #m_ap = calculate_ap_from_pr(p, r)
        mAP = calculate_mAP(self.classified_keypoints, self.total_ground_truth_keypoints)
        AKD, KD_percentile = calculate_AKD(self.classified_keypoints, self.total_ground_truth_keypoints)   
        return mAP, AKD, KD_percentile
...
class KeypointAPMetrics(Metric):
    ...
    def update(self, detected_keypoints: List[DetectedKeypoint], gt_keypoints: List[Keypoint], scale_factor: torch.Tensor):
        for metric in self.ap_metrics:
            metric.update(detected_keypoints, gt_keypoints, scale_factor)

    def compute(self) -> Dict[float, float]:
        result_dict = {}
        for metric in self.ap_metrics:
            mAP, AKD, kd_percentile = metric.compute()
            result_dict.update({metric.keypoint_threshold_distance: (mAP, AKD)})
        return result_dict, kd_percentile
