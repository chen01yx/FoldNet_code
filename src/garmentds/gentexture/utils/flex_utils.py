        ...
        for i in range(self.num_picker):
            ...
            if pick_flag[i]:
                ...
                if self.picked_particles[i] is None:
                    ...
                    #if i == 0:
                    #    print(f"[ INFO ] Try to pick particles: picker pos:{picker_pos[i]}"
                    #          f", threshold:{self.picker_threshold}, particle_radius:{self.particle_radius}"
                    #          f", min_dist:{np.min(dists)}, mask_size:{len(idx_dists)}")
...
class PickerPickPlace(Picker):
    ...
    def step(self, action, target_coverage=None, 
             step_sim_fn=lambda: pyflex.step(),
             dump_visualizations=False, output_dir="."):
        """
        action: Array of picker_num x 4. For each picker,
        ...
        for i in range(int(min(num_step, self.steps_limit))):
            ...
            if dump_visualizations:
                visualize_cloth(os.path.join(output_dir, f"{self.moved_steps}.png"))
                trimesh.PointCloud(vertices=pyflex.get_positions().reshape(-1, 4)[:, :3]).export(os.path.join(output_dir, f"{self.moved_steps}.ply"))
                self.moved_steps += 1
                
            total_steps += 1
            if target_coverage is not None:
                coverage = get_current_covered_area()
...
####################################################
# The following functions are used to set up scene #
####################################################

def get_default_config(
    particle_radius=0.00625,#0.0175,
    cloth_pos = [0.0, 1.0, 0.0],
    cloth_size = [int(0.6/0.0175), int(0.368/0.0175)],
    cloth_stiffness = (0.75, .02, .02),
    camera_position = [0, 2, 0],
    camera_angle = [np.pi*0.5, -np.pi*0.5, 0],
    camera_size = [480, 480],
    camera_fov = 39.5978 / 180 * np.pi,
    collisionDistance = 0.0006 * 3,
    scale=1.0,
):
    config = {
        'scale':scale,
            ...
            'default_camera':
                {
                    ...
                    'cam_size': camera_size,
                    'cam_fov': camera_fov,
                }
            },
        'scene_config': {
            'radius': particle_radius * scale * 3,
            'buoyancy': 0,
            'numExtraParticles': 80000,
...
def load_cloth(cloth_obj_path : str):
    cloth_mesh_raw = trimesh.load_mesh(cloth_obj_path, process=False) # has overlapping verts
    cloth_mesh = trimesh.Trimesh(cloth_mesh_raw.vertices, cloth_mesh_raw.faces)
    
    # map raw verts to processed verts
    verts = cloth_mesh_raw.vertices.copy()
    verts_processed = cloth_mesh.vertices.copy()
    dist = np.linalg.norm(verts.reshape(1,-1,3) - verts_processed.reshape(-1,1,3), axis=2)
    dist_min = dist.argmin(axis=0)
    
    # center cloth
    border = cloth_mesh.vertices.max(axis=0) - cloth_mesh.vertices.min(axis=0)
    scale = (2.0 / np.array(border)).min()
    shift = (cloth_mesh.vertices.max(axis=0) + cloth_mesh.vertices.min(axis=0)) / 2
    cloth_mesh.vertices = cloth_mesh.vertices - shift
    triangle_faces = cloth_mesh.faces.copy()
    stretch_edges, shear_edges, bend_edges = set(), set(), set()
    
    # Stretch & Shear
    for face in cloth_mesh.faces:
        ...
        shear_edges.add(tuple(sorted([face[0], face[1]])))
        shear_edges.add(tuple(sorted([face[1], face[2]])))
        shear_edges.add(tuple(sorted([face[2], face[0]])))
    
    # Bend
    neighbours = dict()
    for vid in range(cloth_mesh.vertices.shape[0]):
        neighbours[vid] = set()
    ...
    if(np.random.rand() < 0.5):
        flipped = True
        cloth_mesh.apply_transform(tra.euler_matrix(*[-np.pi/2, 0, np.pi], 'sxyz'))
    else:
        cloth_mesh.apply_transform(tra.euler_matrix(*[-np.pi/2, 0, 0], 'sxyz'))
    rotate_y = np.random.uniform(0, 2*np.pi)
    cloth_mesh.apply_transform(tra.euler_matrix(*[0, rotate_y, 0], 'sxyz'))

    return flipped, dist_min, cloth_mesh, \
...
#######################################################################
# The following functions are for measuring the coverage of the cloth #
#######################################################################
...
######################################################
# The following functions are used during simulation #
######################################################
...
def wait_until_stable(
        ...
        visualize_start_step: int = 0,
        output_dir: str = ".",
        step_sim_fn=lambda: pyflex.step()
)-> tuple[bool, int]:
    
    step = visualize_start_step
    for _ in range(max_steps):
        ...
        step_sim_fn()
        if gui:
            pyflex.render(os.path.join)

        if dump_visualizations:
            visualize_cloth(os.path.join(output_dir, f"{step}.png"))
            trimesh.PointCloud(vertices=pyflex.get_positions().reshape(-1, 4)[:, :3]).export(os.path.join(output_dir, f"{step}.ply"))
            step += 1
...
def visualize_cloth(
    output_path: str = "./cloth.png", 
    camera_height=480,
    camera_width=480, 
):
    rgb, _, _ = pyflex.render()
    # Need to reverse the height dimension
    rgb = np.flip(rgb.reshape([camera_height, camera_width, 4]), 0)[:, :, :3].astype(np.uint8)
