...
@dataclass
class HoodedCfg(GarmentCfgTypeA):
    """
    - sh = shoulder
    - cl = collar
    - nf = neck_f
    ...
    l_neck_f: Point2 = Point2(-0.01, +0.27)
    r_neck_f: Point2 = Point2(+0.01, +0.27)
    ...
    l_nf_cl: list[Point2] = field(default_factory=lambda:[Point2(-0.04, +0.28)])
    r_nf_cl: list[Point2] = field(default_factory=lambda:[Point2(+0.04, +0.28)])
    l_nb_cl: list[Point2] = field(default_factory=lambda:[Point2(-0.04, +0.26)])
    r_nb_cl: list[Point2] = field(default_factory=lambda:[Point2(+0.04, +0.26)])
    ...
    def symmetry(self):
        ...
        for r_keypoint in [
            ...
            "r_spine_bottom", "r_neck_f",
        ]:
            setattr(self, r_keypoint, mirror(*getattr(self, "l_" + r_keypoint[2:])))
        
        for keypoint in ["spine_bottom_b", "neck_b", "hood_top"]:
...
class Hooded(GarmentTypeA):
    _meta = dict(name="hooded")
    all_part_name_type = Literal["frontl", "frontr", "back", "leftf", "leftb", "rightf", "rightb", "hood"]
    all_part = dict(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(cfg=utils.dataclass_init_from_dict(HoodedCfg, kwargs))
        self._cfg: HoodedCfg
    
    def _draw(self, width: int, height: int, xy2ij: Callable[[float, float], tuple[int, int]], cfg: HoodedCfg):
        ...
        img_np[np.where(mask_frontl)] = (0, 127, 127, 255)
        img_np[np.where(mask_frontr)] = (0, 0, 255, 255)
    ...
    def _calculate_uv(
        ...
        if part_name in ["frontl", "frontr", "leftf", "rightf"]:
