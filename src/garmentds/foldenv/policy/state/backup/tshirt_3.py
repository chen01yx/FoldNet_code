...
@dataclass
class FoldStateTShirtPolicy3Cfg(FoldStatePolicyCfg):
    ...
    fold1_z_move: float = PICKER_Z + 0.1
    fold1_z_put: float = PICKER_Z + 0.04
    fold1_z_move_proportional_to_dist: float = 0.1
    fold1_x_max_barrier: float = 0.06 # avoid gripper collision

    fold2_z_grasp: float = PICKER_Z
    fold2_z_move: float = PICKER_Z + 0.16
    fold2_z_put: float = PICKER_Z + 0.04
    fold2_stretch_ratio: float = 0.05
...
class FoldStateTShirtPolicy3(FoldStatePolicy):
    def __init__(self, cfg: FoldStateTShirtPolicy3Cfg, env: FoldEnv):
        super().__init__(cfg, env)
        self._cfg: FoldStateTShirtPolicy3Cfg
    ...
    def _add_action_fold_garment(self, fold_step: int):
        ...
        if fold_step == 0:
            ...
            upper_points = np.array([curr_vert[kpid[lsh]][:2], curr_vert[kpid[rsh]][:2]])
            lower_points = np.array([curr_vert[kpid[lcn]][:2], curr_vert[kpid[rcn]][:2]])
        
            fold_left2right = np.random.rand() < 0.5
            if fold_left2right:
                pick_idx = np.argmax(upper_points[:, 0])
                pick_points = np.stack([upper_points[pick_idx], lower_points[pick_idx]], axis=0)
                put_points = np.stack([upper_points[1 - pick_idx], lower_points[1 - pick_idx]], axis=0)
                l_idx = np.argmax(pick_points[:, 1])
            else:
                pick_idx = np.argmin(upper_points[:, 0])
                pick_points = np.stack([upper_points[pick_idx], lower_points[pick_idx]], axis=0)
                put_points = np.stack([upper_points[1 - pick_idx], lower_points[1 - pick_idx]], axis=0)
                l_idx = np.argmin(pick_points[:, 1])

            delta_x = put_points.mean(axis=0) - pick_points.mean(axis=0)
            xyl_1, xyr_1 = pick_points[l_idx], pick_points[1 - l_idx]
            delta_y = (xyl_1 - xyr_1) * cfg.fold2_stretch_ratio
            xyl_3, xyr_3 = xyl_1 + delta_x + delta_y, xyr_1 + delta_x - delta_y
            xyl_2 = (xyl_1 + xyl_3) / 2
            xyr_2 = (xyr_1 + xyr_3) / 2
