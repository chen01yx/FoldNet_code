...
@dataclass
class FoldStateTShirtLRPolicyCfg(FoldStatePolicyCfg):
...
class _FoldStateTShirtLRPolicy(FoldStatePolicy):
    LEFT_OR_RIGHT: Literal["left", "right"]
    def __init__(self, cfg: FoldStateTShirtLRPolicyCfg, env: FoldEnv):
        super().__init__(cfg, env)
        self._cfg: FoldStateTShirtLRPolicyCfg
        self._keypoint_names_for_shape_match = ["l_shoulder", "l_corner", "r_corner", "r_shoulder"]
        assert self.LEFT_OR_RIGHT in ["left", "right"]
    ...
    def _add_action_rotate_garment(self, rotate_step: int):
        ...
        if rotate_step == 0:
            ...
            xyl_end = xyl_end + np.array([0., -0.15])
            xyr_end = xyr_end + np.array([0., -0.15])
            ...
            xyl_end += np.array([0., -0.10])
            xyr_end += np.array([0., -0.10])
        else:
    ...
    def _add_action_fold_garment(self, fold_step: int):
        ...
        if fold_step == 0:
            ...
            lcn_x, lcn_y = curr_vert[kpid["l_corner"]][:2]
            rcn_x, rcn_y = curr_vert[kpid["r_corner"]][:2]
            lsh_x, lsh_y = curr_vert[kpid["l_shoulder"]][:2]
            rsh_x, rsh_y = curr_vert[kpid["r_shoulder"]][:2]
            ...
            xyc = np.mean(xys, axis=0, keepdims=True)
            tr = np.argmax((xys - xyc) @ np.array([1., 1.])) # top-right
            tl = np.argmax((xys - xyc) @ np.array([-1., 1.])) # top-left
            br = np.argmax((xys - xyc) @ np.array([1., -1.])) # bottom-right
            bl = np.argmax((xys - xyc) @ np.array([-1., -1.])) # bottom-left

            if self.LEFT_OR_RIGHT == "left":
                xyl_1, xyr_1, xyl_3, xyr_3 = xys[tl], xys[bl], xys[tr], xys[br]
            elif self.LEFT_OR_RIGHT == "right":
                xyl_1, xyr_1, xyl_3, xyr_3 = xys[br], xys[tr], xys[bl], xys[tl]
            else:
                raise ValueError(f"Invalid LEFT_OR_RIGHT {self.LEFT_OR_RIGHT}")
...
class FoldStateTShirtLPolicy(_FoldStateTShirtLRPolicy):
    LEFT_OR_RIGHT = "left"


class FoldStateTShirtRPolicy(_FoldStateTShirtLRPolicy):
    LEFT_OR_RIGHT = "right"