...
public:
    ...
    void Initialize(py::dict scene_params)
    {
        ...
        for (auto item : scene_params){
            ...
            else if (key == "radius") {g_params.radius = std::stof(py::str(item.second)); }
            else if (key == "collisionDistance") {g_params.collisionDistance = std::stoi(py::str(item.second)); }
            else if (key == "buoyancy") {g_params.buoyancy = std::stof(py::str(item.second)); }
            else if (key == "dynamicFriction") {g_params.dynamicFriction = std::stof(py::str(item.second)); }
            else if (key == "particleFriction") {g_params.particleFriction = std::stof(py::str(item.second)); }
            else if (key == "dt") {g_dt = std::stof(py::str(item.second)); }
            else if (key == "numIterations") {g_params.numIterations = std::stoi(py::str(item.second)); }
            else if (key == "numSubsteps") {g_numSubsteps = std::stoi(py::str(item.second)); }
            else if (key == "gravity") {g_params.gravity[1] = -std::stof(py::str(item.second)); }
            else {
                std::cout << "Invalid key in scene_params: " << key << std::endl;
                assert(false);
            }
        }
