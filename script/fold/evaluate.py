...
def get_args():
    ...
    parser.add_argument("--model_name", type=str, choices=["dp", "act"], required=True)
    parser.add_argument("--render_set", type=str, default="valid", choices=["train", "valid"])
    parser.add_argument("--device_start", "-s", type=int, default=0)
    parser.add_argument("--device_end", "-e", type=int, default=8)
    parser.add_argument("--category", "-c", type=str, default="tshirt")
    parser.add_argument("--style", "-y", type=int, default=0)
...
class Cfg:
    ...
    traj_num_per_running: int = 1
    render_process_num: int = 1

    cloth_obj_path_fn: Callable[[int], str] = field(
        init=False,
        default=lambda self, i: f"data/fold/mesh/gen_proc/tshirt_sp_20250307/{i % 20}/mesh.obj", 
    )
...
class Main:
    def __init__(self, cfg: Cfg, args):
        self.args = args
    ...
    def run_job(self, job: Job, **kwargs):
        cmd = (
            ...
            + f"env.render_set={self.cfg.render_set} "
            + f"policy.run.name=visual " 
            + f"policy.run.visual.name={self.cfg.model_name} "
            + f"policy.run.visual.ckpt={self.cfg.ckpt} "
        )
        print(f"run command:\n{cmd}")
    ...
    def run(self):
        ...
        dir_prefix = f"data/fold/eval_{self.cfg.difficulty}/{self.cfg.render_set}/{self.cfg.category}{self.cfg.style}/{now_str}"
        ...
        with open(os.path.join(dir_prefix, ".tmp", "info.json"), "w") as f:
            json.dump(dict(pid=os.getpid(), args=self.args), f, indent=4)
...
def main():
    args = get_args()
    Main(Cfg(
        ckpt=args["ckpt"], model_name=args["model_name"], render_set=args["render_set"],
        cudas=list(range(args["device_start"], args["device_end"])),
        category=args["category"], style=args["style"], 
    ), args).run()
