...
def get_args():
    ...
    parser.add_argument("--render_set", type=str, default="train", choices=["train", "valid"])
...
class Main:
    ...
    def run_job(self, job: Job, **kwargs):
        cmd = (
            ...
            + f"policy.run.name=hybrid " 
            + f"policy.run.hybrid.name={self.cfg.model_name} "
            + f"policy.run.hybrid.ckpt={self.cfg.ckpt} "
        )
        print(f"run command:\n{cmd}")
    ...
    def run(self):
        ...
        dir_prefix = f"data/fold/loop_{self.cfg.difficulty}/{self.cfg.category}{self.cfg.style}/{now_str}"
